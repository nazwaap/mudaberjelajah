...
class WTE_Booking_Emails extends WP_Travel_Engine_Emails {
	...
	public function send() {
		...
		$enable_customer_notification = $global_settings[ 'email' ][ 'enable_cust_notif' ] ?? 'yes';

		if ( 'admin' === $this->sendto && isset( $global_settings['email']['disable_notif'] ) && '1' === $global_settings['email']['disable_notif'] ) {
			return;
		}
		if ( 'customer' === $this->sendto && 'no' === $enable_customer_notification ) {
			return;
		}
	 ...
	 * @return void
	 */
	public function replace_subject_tags( $subject ) {
		$template_tags = new Template_Tags( $this->booking->ID, $this->payment->ID );
