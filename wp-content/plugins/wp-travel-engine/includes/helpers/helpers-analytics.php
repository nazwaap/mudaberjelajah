...
function wptravelengine_analytics_queries_with_date( $start_date, $end_date, $query ) {
	...
	if ( $time_format ) {
		...
		$queries_data = $wpdb->get_results( $queries_data );
	}
	...
	if ( 'top_customer' === $query ) {
		...
		$serialized_data = $wpdb->prepare(
			...
			SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value, ';', 18), ':', -1) AS first_name, 
			SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value, ';', 20), ':', -1) AS last_name, 
		...
		$serialized_data = $wpdb->get_results( $serialized_data );
...
function wptravelengine_analytics_queries_without_date( $query ) {
	...
	$queries_data = array();
	if ( 'total' === $query ) {
		$queries_data['range'] = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT
		...
		$queries_data['chart'] = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT
				SUM(pm.meta_value) AS total_amount,
				SUM(pm.meta_value) / (COUNT(DISTINCT p.ID)) AS average_order_value,
	...
	if ( 'top_customer' === $query ) {
		...
		$serialized_data = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT cn.post_id AS customer_id, 
					SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value, ';', 16), ':', -1) AS first_name, 
					SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value, ';', 18), ':', -1) AS last_name, 
					CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(pm.meta_value, ':', 2), ':', -1) AS DECIMAL(10,0)) AS bookings 
			FROM {$wpdb->postmeta} AS cn
			LEFT JOIN {$wpdb->postmeta} AS pm ON pm.post_id = cn.post_id 
...
function wptravelengine_analytics_overview_datefilter_query( $filter_type ) {
	...
	if ( 'today' === $filter_type ) {
		...
		$prev_customers = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT COUNT(*) AS customer_count, DATE(post_date) AS days
				FROM {$wpdb->posts}
				WHERE post_type = %s
		...
		$prev_bookings = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT COUNT(DISTINCT p.ID) AS total_bookings
				FROM {$wpdb->posts} p
				WHERE p.post_type = %s
	...
	if ( 'this_month' === $filter_type ) {
		...
		$totals       = $wpdb->get_results( $totals_query );
		...
		$aov       = $wpdb->get_results( $aov_query );
		...
		$refunds       = $wpdb->get_results( $refunds_query );
		...
		$prev_totals       = $wpdb->get_results( $prev_totals_query );
		...
		$prev_aov       = $wpdb->get_results( $prev_aov_query );
		...
		$prev_customers       = $wpdb->get_results( $prev_customers_query );
		...
		$prev_refunds       = $wpdb->get_results( $prev_refunds_query );
		...
		$prev_bookings       = $wpdb->get_results( $prev_bookings_query );
	}
...
function wptravelengine_analytics_trips_datefilter_query( $filter_type ) {
	...
	if ( 'today' === $filter_type ) {
		...
		$top_performer = $wpdb->get_results(
			"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(order_trips.meta_value, ';', 3),':', -1) AS trip_id, 
					SUM(IF(paid_amount.meta_key = 'paid_amount', paid_amount.meta_value, 0)) as total_earnings 
			FROM {$wpdb->postmeta} AS order_trips 
			LEFT JOIN {$wpdb->postmeta} AS paid_amount ON order_trips.post_id = paid_amount.post_id 
...
function wptravelengine_analytics_customers_datefilter_query( $filter_type ) {
	...
	if ( 'today' === $filter_type ) {
		$total_customer  = $wpdb->get_results(
			"SELECT COUNT(*) AS total_customer 
			FROM {$wpdb->posts} 
			WHERE post_type='customer' 
				AND post_status IN ('publish', 'draft') 
		...
		// Yesterday data for comparison.
		$prev_total_customer = $wpdb->get_results(
			"SELECT COUNT(*) AS total_customer 
			FROM {$wpdb->posts} 
			WHERE post_type='customer' 
				AND post_status IN ('publish', 'draft') 
		...
		$prev_new_customer   = $wpdb->get_results(
			"SELECT COUNT(*) AS customer_count 
			FROM {$wpdb->posts} 
			WHERE post_type='customer' 
				AND post_status IN ('publish', 'draft') 
	...
	if ( 'this_week' === $filter_type ) {
		...
		$serialized_data = $wpdb->get_results(
			"SELECT cn.post_id AS customer_id, SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value,';',18),':',-1) AS first_name, SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value,';',20),':',-1) AS last_name, CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(pm.meta_value,':',2),':',-1) AS DECIMAL(10,0)) AS bookings 
			FROM {$wpdb->postmeta} AS cn 
			LEFT JOIN {$wpdb->postmeta} AS pm ON pm.post_id = cn.post_id 
			WHERE pm.meta_key='wp_travel_engine_bookings' 
				AND cn.meta_key = 'wp_travel_engine_booking_setting' 
	...
	if ( 'last_year' === $filter_type ) {
		...
		$serialized_data = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT cn.post_id AS customer_id, SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value,';',18),':',-1) AS first_name,
			 SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value,';',20),':',-1) AS last_name,
			 CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(pm.meta_value,':',2),':',-1) AS DECIMAL(10,0)) AS bookings
			 FROM {$wpdb->postmeta} AS cn
			 LEFT JOIN {$wpdb->postmeta} AS pm ON pm.post_id = cn.post_id
			 WHERE pm.meta_key='wp_travel_engine_bookings' AND cn.meta_key = 'wp_travel_engine_booking_setting'
			 AND pm.post_id IN (
				 SELECT ID FROM {$wpdb->posts}
				 WHERE post_type='customer' AND post_status IN ('publish', 'draft')
				 AND DATE_FORMAT(post_date, %s) = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 1 YEAR), %s)
...
function wptravelengine_analytics_customers_query( $offset, $per_page, $query ) {
	...
	if ( 'customer_table' === $query ) {
		...
		$serialized_data = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value,';',18),':',-1) AS first_name,
					SUBSTRING_INDEX(SUBSTRING_INDEX(cn.meta_value,';',20),':',-1) AS last_name,
					p.ID AS customer_id,
					p.post_title AS customer_email,
					CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(pm.meta_value,':',2),':',-1) AS DECIMAL(10,0)) AS bookings,
					SUBSTRING_INDEX(SUBSTRING_INDEX(pms.meta_value, '\"cost\";', -1), 's:3:\"due\"', 1) AS cost,
					DATE(p.post_date) AS date
			FROM {$wpdb->postmeta} AS pm
			LEFT JOIN {$wpdb->postmeta} AS pms ON pm.post_id = pms.post_id
			LEFT JOIN {$wpdb->posts} AS p ON pm.post_id = p.ID
			LEFT JOIN {$wpdb->postmeta} AS cn ON pm.post_id = cn.post_id
			WHERE pm.meta_key='wp_travel_engine_bookings'
				AND pms.meta_key='wp_travel_engine_booked_trip_setting'
				AND cn.meta_key = 'wp_travel_engine_booking_setting'
				AND pm.post_id IN (
					SELECT ID FROM {$wpdb->posts}
					WHERE post_type='customer' AND post_status IN ('publish', 'draft')
				)
			ORDER BY bookings DESC, cost DESC
			LIMIT %d, %d",
				$offset,
				$per_page
			)
		);
		$customer_count  = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT COUNT(*) AS total_customer
			FROM {$wpdb->posts}
...
function wptravelengine_analytics_dashboard_data() {
	...
	// Current month data.
	$current_month_data = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT SUM(pm.meta_value) AS total_amount,
				SUM(CASE WHEN pm.meta_key = 'paid_amount' THEN pm.meta_value ELSE 0 END) AS total_earnings,
				COUNT(DISTINCT p.ID) AS total_bookings
	...
	// Last month data.
	$last_month_data = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT SUM(pm.meta_value) AS total_amount,
				SUM(CASE WHEN pm.meta_key = 'paid_amount' THEN pm.meta_value ELSE 0 END) AS total_earnings,
				COUNT(DISTINCT p.ID) AS total_bookings
	...
	$bookings_data = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(pm.meta_value, ';', 18), ':', -1) AS fname,
				SUBSTRING_INDEX(SUBSTRING_INDEX(pm.meta_value, ';', 20), ':', -1) AS lname,
				SUBSTRING_INDEX(SUBSTRING_INDEX(pm.meta_value, ';', 11), ':', -1) AS trip_name,
	...
	// Customer data.
	$current_month_customer_data = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT COUNT(*) AS customer_count
		FROM {$wpdb->posts}
		WHERE post_type = %s
	...
	$prev_month_customer_data = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT COUNT(*) AS customer_count
		FROM {$wpdb->posts}
		WHERE post_type = %s
	...
	$current_year_customer_data = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT COUNT(*) AS customer_count
		FROM {$wpdb->posts}
		WHERE post_type = %s
	...
	$prev_year_customer_data = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT COUNT(*) AS customer_count
		FROM {$wpdb->posts}
		WHERE post_type = %s
	...
	$prev_year_current_month = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT COUNT(*) AS customer_count
		FROM {$wpdb->posts}
		WHERE post_type = %s
...
function wptravelengine_analytics_taxonomy_datefilter_query( $filter_type ) {
	...
	if ( 'today' === $filter_type ) {
		$top_destination = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT
				SUBSTRING_INDEX(SUBSTRING_INDEX(order_trips.meta_value, ';', 3), ':', -1) AS trip_id,
				SUM(IF(paid_amount.meta_key = 'paid_amount', paid_amount.meta_value, 0)) AS total_earnings,
